...
private:
  ...
  Tree null_denotation(const_TokenPtr tok, int assignment_paren);
  ...
  Tree parse_expression(int right_binding_power, int assignment_paren);
...
public:
  ...
  Tree parse_if_statement(Tree variable);
...
{
  const_TokenPtr t = lexer.peek_token();
  //return(t->get_id() == Tiger::END || t->get_id() == Tiger::ELSE || t->get_id() == Tiger::END_OF_FILE || t->get_id () == Tiger::RIGHT_PAREN);
...
{
  return parse_expression(/* right_binding_power */ 0,0);
}

Tree
Parser::parse_expression(int first)
{
  return parse_expression(first,0);
}
...
Parser::parse_expression(int right_binding_power,int assignment_paren)
{
  ...
  Tree expr = null_denotation(current_token,assignment_paren);
...
// "prefix" position
Tree
Parser::null_denotation(const_TokenPtr tok,int assignment_paren) {
  /*
  ...
  case Tiger::LEFT_PAREN:
    {
      ...
      if(assignment_paren==0){
       ...
       /* printf("LEFT_PAREN, TIPO   %s \n ",print_type(expr.get_type()));
        ...
        Tree retorno = Tree(expr, tok->get_locus());
        retorno.set_last_stmt(last_stmt);
        return retorno;
...
Parser::parse_assignment_statement(Tree variable){
  ...
  if(first_of_expr->get_id() != Tiger::IF){
    ...
    //skip_token(Tiger::SEMICOLON);
    //get_current_stmt_list().get_tipo();
    ...
    skip_token(Tiger::IF);
    return parse_if_statement(variable);
  }
}
...
Tree
Parser::build_if_statement(Tree bool_expr, Tree then_part, Tree else_part/*,Tree variable,Tree last_then_stmt,Tree last_else_stmt*/)
{
 ...
 /* if(variable!=0){
    Tree assig_expr = build_tree(MODIFY_EXPR, variable->get_locus(),expr.get_type(), variable, last_then_stmt);
  }*/
  ...
  stmt_list.set_tipo(then_part.get_type());
...
Tree
Parser::parse_if_statement(Tree variable)
{
   /*
...
*/
  ...
  Tree last_stmt_else;
  Tree last_stmt_then = stmt_stack.back();
  stmt_stack.pop_back();
  if(variable!=0){
    Tree assig_expr = build_tree(MODIFY_EXPR, variable.get_locus(),variable.get_type(), variable, last_stmt_then);
  ...
  if(tok->get_id() == Tiger::ELSE){
      ...
      last_stmt_else = stmt_stack.back();stmt_stack.pop_back();
      if(variable!=0){
    ...
    if(variable!=0){
      error_at (variable.get_locus (),"Variable assignment though IF but without ELSE condition");      
      skip_after_end();
      return Tree::error ();
    }
    if (tok->get_id () == Tiger::END){
        skip_token (Tiger::END);
    }else{
        unexpected_token (tok);
        return Tree::error ();
 ...
 /* if(tok->get_id() == Tiger::END){
  ...
  Tree stmt = build_if_statement(expr, then_stmt, else_stmt/*,variable,last_stmt_then,last_stmt_else*/);  
   return stmt;
}
...
*/
 ...
 // skip_token(Tiger::SEMICOLON);
  ...
  if(expr.get_type() == integer_type_node){
    ...
    {
      error_at(first_of_expr->get_locus(), "value of type %s is not a valid print operand", print_type(expr.get_type()));
      return Tree::error();
    }
