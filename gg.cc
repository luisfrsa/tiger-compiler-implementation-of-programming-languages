...
namespace Tiny {
  ...
  struct Parser {
    private: void skip_after_semicolon();
    void skip_after_end();
    ...
    BINARY_HANDLER_LIST# undef BINARY_HANDLER

    public: Parser(Lexer & lexer_): lexer(lexer_),
    puts_fn(),
    printf_fn(),
    scanf_fn() {}
  ...
  void
  Parser::unexpected_token(const_TokenPtr t) {::error_at(t - > get_locus(), "unexpected %s\n", t - > get_token_description());
  }
  ...
  Parser::done_end_or_else() {
    const_TokenPtr t = lexer.peek_token();
    return (t - > get_id() == Tiny::END || t - > get_id() == Tiny::ELSE || t - > get_id() == Tiny::END_OF_FILE);
  }
  ...
  Parser::parse_field_declaration(std::vector < std::string > & field_names) {
    ...
    if (std::find(field_names.begin(), field_names.end(),
        identifier - > get_str()) != field_names.end()) {
  ...
  Parser::coerce_binary_arithmetic(const_TokenPtr tok, Tree * left, Tree * right) {
    ...
    if (left_type == right_type) {
      ...
      if (left_type == integer_type_node) { * left = build_tree(FLOAT_EXPR, left - > get_locus(), float_type_node,
          left - > get_tree());
      } else { * right = build_tree(FLOAT_EXPR, right - > get_locus(),
...
void
tiny_parse_files(int num_files,
  const char * * files) {
